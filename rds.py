...
def main():
    ...
    SQL = '''CREATE TABLE invoices (invoice_id INT, invoice_amount FLOAT, customer_name CHAR(25))'''
    createTable.execute(SQL)

    SQL_INSERT = '''INSERT INTO invoices (invoice_id, invoice_amount, customer_name) VALUES (%s, %s, %s)'''
    SQL_SELECT = """SELECT * FROM invoices WHERE invoice_amount=%s"""
    try:
        with connection.cursor() as cursor:
					    cursor.execute(SQL_INSERT, (1, 88, 'Zeeshan'))
        connection.commit()
        with connection.cursor() as cursor:
					    cursor.execute(SQL_SELECT, ('88'))
